...
namespace gbl {
  ...
  class GblPoint {
    public:
      ...
      unsigned int GetLabel() {return theLabel;}
      const unsigned int & GetLabel() const {return theLabel;}
...
inline std::ostream & operator << (std::ostream & os, const gbl::GblPoint & point) {
  return os 	<< "\n This point's label " << point.GetLabel()
    //<< "\n   - smth = " << point.scatFlag
    << std::endl;
}
#endif /* GBLPOINT_H_ */
